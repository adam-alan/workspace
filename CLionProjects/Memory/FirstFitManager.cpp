...
FirstFitManager::FirstFitManager(int limitSize)
:limitSize(limitSize){

}

void FirstFitManager::memoryInit(int size) {
...
int FirstFitManager::memoryAlloc(int size){
...
bool FirstFitManager::doAlloc(FreeBlock & request) {
...
void FirstFitManager::memoryFree(FreeBlock freeBlock) {
	freeBlockList.sort([](FreeBlock freeBlock1,FreeBlock freeBlock2)->bool {
		return freeBlock1.getStartAddr() < freeBlock2.getStartAddr();
	});
...
std::list<FreeBlock>& FirstFitManager::getfreeBlockList(){
