...
#include "WorstFitManager.h"
#include <iostream>
#include <string>


WorstFitManager::WorstFitManager(int limitSize)
		:limitSize(limitSize){

}

void WorstFitManager::memoryInit(int size) {
...
int WorstFitManager::memoryAlloc(int size){
...
bool WorstFitManager::doAlloc(FreeBlock & request) {
...
void WorstFitManager::memoryFree(FreeBlock freeBlock) {
	freeBlockList.sort([](FreeBlock freeBlock1,FreeBlock freeBlock2)->bool {
		return freeBlock1.getStartAddr() < freeBlock2.getStartAddr();
	});
...
std::list<FreeBlock>& WorstFitManager::getfreeBlockList(){
